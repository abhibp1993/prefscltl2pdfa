...
class PrefScLTL:
    ...
    def __init__(self, spec, alphabet=None, **kwargs):
        ...
        **kwargs
        ...
        if not kwargs.get("skip_parse", False):
            self.parse(
                auto_complete=kwargs.get("auto_complete", "minimal"),
            )

    def __eq__(self, other):
        keys = ["f_str", "atoms", "alphabet", "phi", "relation"]
        if isinstance(other, PrefScLTL):
    ...
    def __repr__(self):
        return f"<PrefScLTL Formula at {id(self)}>"
    ...
    def parse(self, auto_complete="minimal"):
        ...
        ----------
        auto_complete : str, optional
            The method to use for auto-completion (default is "minimal").
        ...
        """
        logger.debug(f"Parsing prefscltl formula with {auto_complete=}: \n{self.raw_spec}")

        # Check auto_complete inputs
        if auto_complete not in ["incomparable", "minimal"]:
            logger.error(
                f"Unknown auto_complete value '{auto_complete}' to parse function. "
                f"Accepted values are {['incomparable', 'minimal']}. "
                f"New auto_complete value is set to 'minimal'."
            )
        ...
        for varphi in phi.values():
            ...
            atoms.update(set(map(str, spot.atomic_prop_collect(varphi))))
    ...
    def translate(self, semantics=semantics_mp_forall_exists, show_progress=False):
        ...
        for i in sorted_phi:
            log_message += f"\n======== {self.phi[i]} ========\n{pprint.pformat(aut.dfa[sorted_phi.index(i)])}\n"
    ...
    def _parse_header(self):
        ...
        if len(header) != 2:
            raise ValueError(f"Ill-formatted specification file. Header should be `prefscltl <num_formula>`, "
                             f"where specifying <num_formula> is optional.")

        if header[0].strip().lower() != "prefscltl":
            raise ValueError(f"Not a PrefScLTL formula. Likely a '{header[0].strip().lower()}' formula.")
    ...
    def _parse_scltl(self, raw_spec):
        ...
        for formula in raw_spec:
            scltl_formula = PARSER(formula.strip())
            phi.append(scltl_formula)
            atoms.update(set(spot.atomic_prop_collect(scltl_formula)))
        return atoms, phi
    ...
    def _construct_preference_graph(self, aut, semantics, show_progress=False):
        ...
        for qid, data in tqdm(
            ...
            if semantics in PrefScLTL.MAXIMAL_SEMANTICS:
            ...
            # print(qid, q, utils.outcomes(aut.dfa, q), outcomes)
        ...
        # pprint.pprint(list(pg.nodes(data=True)))
        ...
        # phi = [self.phi[formula_id] for formula_id in sorted(self.phi.keys())]
        phi = [self.phi[formula_id] for formula_id in sorted([k for k in self.phi.keys() if k != -1])] + [self.phi[-1]]
        state2node = dict()
        for component in scc:
            ...
            cls_name = str(PARSER(cls_name)) if cls_name != "()" else "none"
    ...
    def _construct_spec_ir(self):
        ...
        # Construct phi: the set of ScLTL formulas.
        for i in range(n_phi):
            try:
                # Parse formula
                phi[i] = PARSER(stmts[i].strip())

                # Assert that the formula belongs to ScLTL class
                if phi[i].mp_class().lower() not in ["b", "g"]:
                    raise TypeError(f"Formula {phi[i]} is not a ScLTL formula.")
                ...
                raise SyntaxError(
                    ...
                    f"2. ScLTL formula is ill-formed.\n"
                    f"3. Did you manually add completion formula (which cannot be ScLTL formula)?."
                )

        # Eliminate formulas with equivalent languages
        equiv = self._find_equivalent_scltl(phi)
        compact_phi, replacement = self._process_equiv_scltl(phi, equiv)
    ...
    def _auto_complete(self, phi, spec_ir, auto_complete):
        ...
        .. note:: We reserve ID `-1` for completion formula. The rationale behind this is explained below.
            Since all formulas in `phi` are ScLTL formulas, the set phi is never complete.
            The completion formula is not an ScLTL formula, and may need to be separately processed
            in algorithms operating on PrefScLTL formulas (e.g., during construction of Preference Automaton,
            the DFA for completion formula must not be constructed).
        """
        ...
        logger.debug(f"Intermediate representation after completion: \n{pprint.pformat(log_string)}")
    ...
    def _is_lang_complete(self, phi):
        ...
        if spot.formula.is_tt(equiv_formula):
            return True
        return False

        dfa = utils.scltl2dfa(equiv_formula)
    ...
    def _build_partial_order(self, phi, pref_stmts):
        ...
        logger.debug(
            f"Clauses from intermediate representation:"
            f"\nset_w={pprint.pformat(set_w)} "
            f"\nset_p={pprint.pformat(set_p)} "
            f"\nset_i={pprint.pformat(set_i)} "
            f"\nset_j={pprint.pformat(set_j)} "
        )
        ...
        logger.debug(
            f"Clauses after applying transitive and reflexive closures:"
            f"\nset_p={pprint.pformat(set_p)} "
            f"\nset_i={pprint.pformat(set_i)} "
            f"\nset_j={pprint.pformat(set_j)} "
        )
        ...
        logger.debug(f"Partial order for input specification:"
                     f"\nphi={pprint.pformat(phi)}"
                     f"\nmodel={pprint.pformat(model)}"
                     f"\nmodel={pprint.pformat(log_model)}")
        return phi, model
    ...
    def _find_equivalent_scltl(self, phi):
        ...
        for i in range(len(indices)):
            for j in range(i + 1, len(indices)):
                ...
                # Equivalence formula
                if spot.are_equivalent(phi[indices[i]], phi[indices[j]]):
    ...
    def _pre_to_partial_order(self, phi, preorder):
        ...
        for component in scc:
            if len(component) >= 2:
                ...
                replacement_scltl = PARSER(" | ".join(formulas))

                # Add new formula to phi
                phi[max_index + 1] = replacement_scltl
